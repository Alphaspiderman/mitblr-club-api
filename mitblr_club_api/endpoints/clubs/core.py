...
class ClubsCore(HTTPMethodView):
    ...
    async def post(self, request: Request,body: ClubTeam_Create, club_slug: Optional[str]):
        ...
        student = await students.find_one({"application_number":body.application_number})
        ...
        if await club_teams.find_one({"student_id":student["_id"],"position":body.position,"club":body.club}) is not None:
             return json(
                {"status": 409, "error": "Conflict", "message": "Student already exists in club_teams collection."},
                status=409
             )
